...
mod tests {
    ...
    impl MockBridge {
        fn new() -> (Self, Arc<Mutex<Vec<Task>>>) {
            ...
            (Self { submitted: submitted.clone() }, submitted)
        }
    }
    impl Bridge for MockBridge {
        ...
        fn query_status(&self, _task_id: &Uuid) -> BifrostResult<TaskStatusResponse> {
            Ok(TaskStatusResponse { task_id: Uuid::nil(), status: TaskStatus::Completed, message: None })
        }
        fn get_result(&self, _task_id: &Uuid) -> BifrostResult<TaskResult> {
            Err(BifrostError::TaskNotFound(Uuid::nil()))
        }
        fn list_pending_tasks(&self) -> BifrostResult<Vec<Task>> { Ok(vec![]) }
        fn read_task(&self, _task_id: &Uuid) -> BifrostResult<Task> {
            Err(BifrostError::TaskNotFound(Uuid::nil()))
        }
        fn write_result(&self, _task_id: &Uuid, _result: &TaskResult) -> BifrostResult<()> { Ok(()) }
        fn write_status(&self, _task_id: &Uuid, _status: &TaskStatus, _message: Option<&str>) -> BifrostResult<()> { Ok(()) }
        fn remove_task(&self, _task_id: &Uuid) -> BifrostResult<()> { Ok(()) }
    }
    ...
    fn test_launch_job_passes_working_dir_and_env() {
        ...
        assert_eq!(t.env_vars.get("MY_VAR").map(|s| s.as_str()), Some("job-env-ok"), "env_vars 必须传递");
