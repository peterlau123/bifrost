...
impl<'a> GpuGuard<'a> {
    fn new(gpu_scheduler: &'a mut GpuScheduler, gpu_id: u32, task_id: Uuid) -> Self {
        Self { gpu_scheduler, gpu_id, task_id, released: false }
    }

    fn release(mut self) -> Result<(), String> {
        self.released = true;
        self.gpu_scheduler.release_gpu(self.gpu_id, self.task_id)
...
impl GpuTaskProcessor {
    pub fn new(
        ...
        Ok(Self { gpu_scheduler: scheduler, executor, batch_tracker })
    }
    ...
    pub async fn process_task(&mut self, task: Task) -> Result<TaskResult, String> {
        ...
        let (scheduled_task, gpu_id) = self.gpu_scheduler.schedule_next()
        ...
        if let Some(batch_id) = task.batch_id {
            if let Some(tracker) = &self.batch_tracker {
                if let Ok(mut progress) = tracker.load_progress(batch_id) {
                    ...
                    progress.completed_tasks.push((result.task_id, result.status.clone(), task_name));
                    progress.updated_at = Utc::now();

                    if progress.completed_tasks.len() == progress.total_tasks {
                        let all_success = progress.completed_tasks.iter().all(|(_, status, _)| *status == TaskStatus::Completed);
                        progress.status = if all_success { BatchStatus::Completed } else { BatchStatus::Failed };
                    }
...
mod tests {
    ...
    fn create_test_task(name: &str) -> Task {
        Task::new(format!("echo {}", name), TaskType::Shell)
            .with_timeout(5).with_working_dir(PathBuf::from("."))
    }
