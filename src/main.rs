...
enum ClientCommand {
    ...
    /// Launch a job from a YAML file (sequential task execution)
    Launch {
        #[arg(short, long)]
        job: PathBuf,
    },
...
fn handle_client_mode(command: ClientCommand) {
    ...
    match command {
        ...
        ClientCommand::Launch { job } => {
            let protocol = Protocol::new(shared_storage.clone()).expect("Failed to create protocol");
            match bifrost::core::job::load_job(&job) {
                Ok(job_def) => {
                    match bifrost::client::launcher::launch_job(&protocol, db.as_ref(), job_def) {
                        Ok(job_result) => { println!(); println!("{}", serde_json::to_string_pretty(&job_result).unwrap()); }
                        Err(e) => eprintln!("Job failed: {}", e),
                    }
                }
                Err(e) => eprintln!("Bad job: {}", e),
            }
        }
    ...
    let settings = if let Some(ref config_path) = config {
        match std::fs::read_to_string(config_path) {
            Ok(content) => match serde_json::from_str::<bifrost::core::settings::BifrostSettings>(&content) {
                Ok(s) => s, Err(_) => { eprintln!("Bad config, using defaults"); bifrost::core::settings::BifrostSettings::defaults() }
            },
            Err(_) => { eprintln!("Cannot read config, using defaults"); bifrost::core::settings::BifrostSettings::defaults() }
        }
    } else { settings::load() };
    if systemd { println!("  Mode: systemd service"); }
    println!("  Shared storage: {}", settings.shared_storage.display());
    let shutdown = Arc::new(AtomicBool::new(false));
    let s = shutdown.clone();
    ctrlc::set_handler(move || { eprintln!("
Shutdown requested..."); s.store(true, Ordering::SeqCst); }).expect("Error setting Ctrl+C handler");
    let rt = tokio::runtime::Runtime::new().expect("Failed to create runtime");
    rt.block_on(async { if let Err(e) = run_daemon(settings, shutdown).await { eprintln!("Daemon error: {}", e); } });
